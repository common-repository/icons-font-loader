...
class BIFL_ZIP_Uploader {
	...
	protected $is_uicons = false;
	protected $targeted_folder = '/font';
	...
	public function upload( $file ) {
		...
		if( move_uploaded_file( $temp_name, $working_dir . "/" . $zip_file ) ){
			...
			$extra_folder = '';
			$folders = scandir($upload_path);
			if(isset($folders[2]) && $folders[2] !== 'font' && $wp_filesystem->is_dir( $upload_path.'/'. $folders[2] )){
				$extra_folder = '/'.$folders[2];
			}

			if(!$wp_filesystem->is_dir( $upload_path.$extra_folder.$this->targeted_folder )){
				
				if($wp_filesystem->exists($upload_path.$extra_folder.'/how-to-use.html')){
					$this->is_uicons = true;
					$this->targeted_folder = '/css';
				}else {
					$wp_filesystem->delete( $upload_path, true );
					$wp_filesystem->delete( $working_dir, true );
			...
			$files = scandir($upload_path.$extra_folder.$this->targeted_folder);
			...
			// get the final path of preview file and the css file
			
			$upload_dir = wp_upload_dir();
			$upload_url = trailingslashit( $upload_dir['baseurl'].'/'.$this->folder.'/'. str_replace('.', '-',  $this->item_folder.$extra_folder.$this->targeted_folder));
			$css_file_path = '';
			$preview_path = '';
			$status = 'active';
			$files = scandir ($upload_path.$extra_folder.$this->targeted_folder);
			foreach($files as $file){
				...
				if($this->is_uicons){
					$preview_path = trailingslashit( $upload_dir['baseurl'].'/'.$this->folder.'/'.str_replace('.', '-', $this->item_folder.$extra_folder)).'how-to-use.html';
				}
			}
